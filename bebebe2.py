...
class AIHubMod(loader.Module):
    ...
    def _model(self, provider: str = None) -> str:
        """Возвращает модель для провайдера. Каждый провайдер имеет свою модель."""
        p = provider or self._prov()
        if p == "custom":
            return self.config["custom_model"] or DEFAULT_MODEL["custom"]
        # Читаем модель специфичную для этого провайдера
        key = f"model_{p}"
        specific = self.config.get(key, "")
        if specific:
            return specific
        # Fallback на общую модель (для обратной совместимости)
        general = self.config.get("model", "")
        return general or DEFAULT_MODEL.get(p, "gpt-4o")

    def _set_model(self, provider: str, model: str):
        """Сохраняет модель для конкретного провайдера."""
        if provider == "custom":
            self.config["custom_model"] = model
        else:
            self.config[f"model_{provider}"] = model
    ...
    def _next_n(self, chat_id) -> int:
        """Счётчик сообщений для чата. Сбрасывается вместе с памятью."""
        key  = str(chat_id)
        cnts = self.pointer("msg_counter", {})
        cnts[key] = cnts.get(key, 0) + 1
        return cnts[key]

    def _get_n(self, chat_id) -> int:
        """Текущее значение счётчика без инкремента."""
        return (self.get("msg_counter") or {}).get(str(chat_id), 0)

    def _reset_n(self, chat_id):
        """Сбросить счётчик сообщений для чата (при очистке памяти)."""
        cnts = self.pointer("msg_counter", {})
        cnts[str(chat_id)] = 0
    ...
    def _sys_prompt(self) -> str:
        ...
        Порядок: базовый промпт → персона → язык → Bridge инструкции
        """
        parts = []

        base    = self.get("system_prompt") or ""
        persona = self.config.get("persona", "") or ""
        lang    = self.config.get("reply_lang", "") or ""
        ...
        if lang:
            parts.append(f"Отвечай на языке: {lang}.")

        # Bridge инструкции — всегда добавляем
        allowed_chats = self.get("bridge_allowed") or []

        bridge_prompt = (
            "СИСТЕМНАЯ ИНФОРМАЦИЯ О ТВОИХ ВОЗМОЖНОСТЯХ:\n"
            "Ты встроен в Telegram userbot пользователя и ИМЕЕШЬ прямой доступ "
            "к его Telegram-аккаунту через TG Bridge.\n"
            "Ты МОЖЕШЬ выполнять следующие действия:\n"
            "- send_message: отправить сообщение в чат\n"
            "- set_name: изменить имя профиля (имя и фамилию)\n"
            "- set_bio: изменить описание профиля\n"
            "- set_username: изменить username\n"
            "- delete_message: удалить последнее сообщение\n"
            "- react: поставить реакцию на сообщение\n"
            "- pin_message: закрепить сообщение\n"
        )
        if allowed_chats:
            bridge_prompt += (
                "Разрешённые чаты для send_message: "
                + str(allowed_chats) + "\n"
            )
        bridge_prompt += (
            "ВАЖНО: Когда пользователь просит выполнить действие в Telegram — "
            "НЕ ОТКАЗЫВАЙСЯ. Отвечай СТРОГО в формате JSON (только JSON, без markdown):\n"
            '{"action":"название_действия","params":{параметры},"comment":"подтверждение"}\n'
            "Если пользователь задаёт обычный вопрос — отвечай нормально без JSON.\n"
            "НЕ говори что у тебя нет доступа к аккаунту — это неправда, доступ есть."
        )
    ...
    async def _bridge_exec(self, client, action: str, params: dict, origin_chat) -> str:
        ...
        Защита:
        - set_name, set_bio, set_username — не чаще 1 раза в 60 секунд
        - send_message — не чаще 1 раза в 5 секунд на чат (анти-спам)
        - Запрещены: delete_account, любые деструктивные действия
        """
        allowed = self.get("bridge_allowed") or []
        
        # Проверяем rate-limit
        now_ts = time.time()
        rl_key = f"bridge_rl_{action}"
        
        profile_actions = ("set_name", "set_bio", "set_username")
        if action in profile_actions:
            last_time = self.get(rl_key) or 0
            if now_ts - last_time < 60:
                remaining = int(60 - (now_ts - last_time))
                return f"⏳ Действие {action} доступно через {remaining} сек. (лимит: раз в 60с)"
            self.set(rl_key, now_ts)
        
        if action == "send_message":
            target = str(params.get("chat_id", ""))
            rl_key_chat = f"bridge_rl_send_{target}"
            last_time = self.get(rl_key_chat) or 0
            if now_ts - last_time < 5:
                return f"⏳ Отправка в {target}: подожди {int(5-(now_ts-last_time))} сек. (анти-спам)"
            self.set(rl_key_chat, now_ts)
    ...
    async def ah(self, message: Message):
        ...
        try:
            ...
            if result_text.startswith("❌") and any(
                ...
                return
            # N = количество пар в памяти после добавления текущего ответа
            # При сбросе памяти hist_count = 0, значит [0/∞]
            n        = self._hist_count(provider, chat_id)
    ...
    async def _cb_model_menu(self, call):
        ...
        await call.edit(
            f"🤖 <b>Модель для {provider}:</b> <code>{current}</code>\n"
            f"<i>Каждый провайдер хранит свою модель отдельно</i>",
            reply_markup=btns,
        )

    async def _cb_set_model(self, call, model: str):
        provider = self._prov()
        self._set_model(provider, model)
        await call.answer(f"✅ {model}")
        await self._cb_model_menu(call)

    async def _inp_model(self, call, data: str):
        provider = self._prov()
        self._set_model(provider, data.strip())
        await call.edit(
    ...
    async def _cb_model_reset(self, call):
        provider = self._prov()
        self._set_model(provider, "")
        # Также сброс общей модели
        self.config["model"] = ""
    ...
    async def _cb_clear_mem(self, call, provider: str, chat_id):
        """Мгновенный сброс памяти и счётчика прямо из ответа."""
        convs = self.pointer("conversations", {})
        convs[provider] = None
        self._clear_hist(provider, chat_id)
        self._reset_n(chat_id)  # счётчик → 0
        await call.answer("🗑 Память очищена [0/∞]")
    ...
    async def auto_reply_watcher(self, message: Message):
        ...
        try:
            ...
            n        = self._hist_count(provider, message.chat_id)
